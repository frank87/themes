...
$id = uniqid( 'section-' );
$bg_color = $bg_image = $bg_video_data_attr = $extra_classes = $margin_bottom = $section_padding = $overlay_style = $box_sizing = $bg_attachment = $stretch = '';

$padding = 0;

if(isset($atts['section_padding']) && $atts['section_padding'] != 'use-default-padding') {
...
if($atts['background_options']['background'] == 'image' && !empty($atts['background_options']['image']['background_image']['data']) ){
	...
	if(isset($atts['background_options']['image']['attachment'])) {
		$bg_attachment = 'background-attachment:' . $atts['background_options']['image']['attachment'] . ';';
	}
}
...
if($atts['background_options']['background'] == 'slider') { 
	...
	$extra_classes .= ' liberty_section_with_slides ';
}
...
<section id="<?php echo esc_attr($custom_id); ?>" class="liberty_section <?php echo esc_attr($extra_classes); ?> <?php echo esc_attr($custom_class); ?> tbWow fadeIn" style="<?php echo '' . $bg_color; ?> <?php echo '' . $bg_attachment; ?> <?php echo '' . $bg_image; ?> <?php echo '' . $margin_bottom; ?> <?php echo '' . $box_sizing; ?>" <?php echo '' . $bg_video_data_attr; ?> >
	<?php echo '' . $overlay_style; ?>

	<?php
	if (!empty($slider_images)) {
		foreach ($slider_images as $slider_image) :
		echo '<div class="liberty_section_slide" style="' . $slider_image . '"></div>';
		endforeach;
	}
	?>

	<?php if ( isset($atts['is_boxed']) && $atts['is_boxed'] != 'No') : // BOXED CONTENT ?>
		<div class="container" <?php if (!empty($section_padding)) { echo "style='$section_padding'";} ?>><div class="row">
	<?php else : ?>
		<div class="liberty_no_container" <?php if (!empty($section_padding)) { echo "style='$section_padding'";} ?>>
	<?php endif; ?>
		<?php if ( isset($atts['is_stretched']) && $atts['is_stretched'] ) : // STRETCHED CONTENT ?>
