...
if ($type_of_content == 'product_cat') {
	$args = array(
	    'hide_empty' => true,
	);
	...
	if ($count > 0) {
		...
		<div class="tb-portfolio-caption" <?php echo '' . $caption_style; ?>>
			...
			<?php if (!empty($caption_page) && intval($caption_page)) { ?>
				...
				<a href="<?php echo get_permalink(intval($caption_page)); ?>" style="color: <?php echo esc_attr($a_color); ?>"><?php echo liberty_default_array($atts, 'view_more_text', 'View More'); ?></a>
				</div>
			<?php } ?>
		</div>
		...
		<div class="tb-portfolio-carousel data-color-hover" <?php if ($a_color) : ?>data-a-color="<?php echo esc_attr($a_color); ?>"<?php endif; ?>>
			...
			$cat_title = $product_category->name;
			$cat_link = get_term_link( $product_category );
			$cat_ID = $product_category->term_id;

			$cat_thumbnail_id = get_woocommerce_term_meta( $cat_ID, 'thumbnail_id', true );

			?>

			<div style="background: url(<?php echo esc_url(wp_get_attachment_url( $cat_thumbnail_id )); ?>);">

				<a href="<?php echo esc_url($cat_link); ?>" title="<?php echo esc_attr($cat_title); ?>" class="absolute100"><?php echo esc_attr($cat_title); ?></a>

				<div class="slide_title tb-half-margin absolutecenter tb-primary-font">
					<h3 class="hidden"><?php echo esc_attr($cat_title); ?></h3>
					<span><?php echo esc_attr($cat_title); ?></span>
				</div>
		...
		</div> <!-- tb portfolio carousel holder -->
...
$featured_image = wp_get_attachment_image_src( get_post_thumbnail_id( $postID) , 'liberty-thumb-xl' ) ;
?>
