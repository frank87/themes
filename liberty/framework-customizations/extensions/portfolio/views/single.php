	...
	<div id="primary" class="content-area col-xs-12">
		...
		<?php
			if ( have_posts() ) :
				...
				$thumbnail_type = (isset($this_post_meta['thumbnail_type']) && !empty($this_post_meta['thumbnail_type'])) ? $this_post_meta['thumbnail_type'] : 'circle';
				...
				<article id="post-<?php echo intval($postID); ?>" <?php post_class("post clearfix"); ?>>
					...
					if ($show_video != 'no' && !empty($video_url)) { // show video
						...
						foreach ($thumbnails as $thumbnail) :
							...
							$thumb_to_show = wp_get_attachment_image_src( $attachment_id, 'liberty-thumb-xl' );
							...
							<div class="tb-issues-holder tb-gallery-list col-xs-6 col-sm-3 tbWow fadeIn" <?php if ($delay) {echo ' data-wow-delay="' . esc_attr($delay) . 's" ';} ?>>
								<div class="ch-item ch-img-1 ch-gallery" style="background-image: url(<?php echo esc_url($thumb_to_show[0]); ?>);">
									<a href="<?php echo esc_url($attachment_url); ?>" class="absolute100"
					                data-rel="prettyPhoto[gallery<?php echo intval($postID); ?>]"><?php _e('View', 'liberty'); ?></a>
