...
if (!function_exists('liberty_woocommerce_image_dimensions')) :

add_action( 'init', 'liberty_woocommerce_image_dimensions', 1 );

function liberty_woocommerce_image_dimensions() {
  	$catalog = array(
		'width' 	=> '300',	// px
		'height'	=> '300',	// px
		'crop'		=> 1 		// true
...
add_action( 'woocommerce_after_single_product_summary', 'liberty_woocommerce_output_related_products', 20);


if (!function_exists('liberty_woocommerce_output_related_products')) :

function liberty_woocommerce_output_related_products() {
	$args = array(
		'posts_per_page' => 4,
		'columns' => 4,
		'orderby' => 'rand'
...
add_action('woocommerce_before_main_content', 'liberty_woo_wrapper_start', 10);
add_action('woocommerce_after_main_content', 'liberty_woo_wrapper_end', 10);

function liberty_woo_wrapper_start() {
...
function liberty_woo_wrapper_end() {
	echo '</div></div>';
...
// breadcrumbs
add_filter( 'woocommerce_breadcrumb_defaults', 'liberty_woo_change_breadcrumb_delimiter' );
function liberty_woo_change_breadcrumb_delimiter( $defaults ) {
