...
function fw_theme_font_url() {
	 ...
	 */
	if ( 'off' !== _x( 'on', 'Lato font: on or off', 'liberty' ) ) {
					...
					<div class="contributor-summary">
						...
						<a class="button contributor-posts-link"
						   href="<?php echo esc_url( get_author_posts_url( $contributor_id ) ); ?>">
							<?php printf( _n( '%d Article', '%d Articles', $post_count, 'liberty' ), $post_count ); ?>
						</a>
					</div>
					<!-- .contributor-summary -->
				</div>
	 ...
	 */ {
		function fw_theme_paging_nav( $wp_query = null ) {
			...
			$links = paginate_links( array(
				...
				'prev_text' => esc_html__( '&larr; Previous', 'liberty' ),
				'next_text' => esc_html__( 'Next &rarr;', 'liberty' ),
			) );
			...
			if ( $links ) :
				...
				<nav class="navigation paging-navigation" role="navigation">
					<h1 class="screen-reader-text"><?php esc_html_e( 'Posts navigation', 'liberty' ); ?></h1>
	 ...
	 */ {
		function fw_theme_post_nav() {
			...
			<nav class="navigation post-navigation" role="navigation">
				<h1 class="screen-reader-text"><?php esc_html_e( 'Post navigation', 'liberty' ); ?></h1>

				<div class="nav-links">
					<?php
					if ( is_attachment() ) :
						previous_post_link( '%link',
							esc_html__( '<span class="meta-nav">Published In</span>%title', 'liberty' ) );
					else :
						previous_post_link( '%link',
							esc_html__( '<span class="meta-nav">Previous Post</span>%title', 'liberty' ) );
						next_post_link( '%link', esc_html__( '<span class="meta-nav">Next Post</span>%title', 'liberty' ) );
					endif;
					?>
				</div>
				<!-- .nav-links -->
	 ...
	 */ {
		function fw_theme_posted_on() {
			if ( is_sticky() && is_home() && ! is_paged() ) {
				echo '<span class="featured-post">' . esc_html__( 'Sticky', 'liberty' ) . '</span>';
			}
