...
// Pretty Photo
if ($prettyPhoto) :
wp_enqueue_style('liberty_pp_css', PARENT_URL . "/inc/prettyPhoto/css/prettyPhoto.css", array( ), false, 'all');
wp_enqueue_script('liberty_pp_js', PARENT_URL . "/inc/prettyPhoto/js/jquery.prettyPhoto.js", false, '3.1.5', true);
endif;
...
wp_enqueue_style( 'liberty_style', PARENT_URL . '/inc/css/themeblossom.css' );
wp_enqueue_script( 'liberty_scripts', PARENT_URL . '/inc/js/themeblossom.js', array('jquery'), '1.0.0', true );

if (file_exists(PARENT_DIR . '/inc/admin' . "/$blog_pref" . 'options.css')) {
	wp_enqueue_style( 'liberty_options', PARENT_URL . '/inc/admin' . "/$blog_pref" . 'options.css' );
} else {
	wp_enqueue_style( 'liberty_options', PARENT_URL . '/inc/admin/default-options.css' );
}
...
if (class_exists('Tribe__Events__Main')) :
	$showExport = liberty_default_array($liberty_theme_options, 'tec-archive-remove-export', 'no');
	...
	$showDatebox = liberty_default_array($liberty_theme_options, 'tec-archive-date-box', 'normal');
	if (is_single() && $showDatebox == 'event_countdown') {
		wp_enqueue_script( 'tb-countdown-plugin', PARENT_URL . '/inc/js/jquery.plugin.min.js', array('jquery'), '2.0.2', true );
		wp_enqueue_script( 'tb-countdown', PARENT_URL . '/inc/js/jquery.countdown.min.js', array('jquery'), '2.0.2', true );
	}
endif;
...
if ( class_exists( 'woocommerce' ) ) {
	...
	wp_register_style( 'liberty_woocommerce', PARENT_URL . '/inc/css/woocommerce.css' );
	wp_enqueue_style( 'liberty_woocommerce' );

	
	// isotope
	wp_enqueue_script('liberty_isotope', PARENT_URL . "/inc/js/jquery.isotope.min.js", array('jquery'), '2.0.0', false);
	wp_enqueue_script('liberty_imagesloaded', PARENT_URL . "/inc/js/jquery.imagesloaded.min.js", array('jquery'), '3.1.4', false);	
